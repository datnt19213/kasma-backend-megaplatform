...
export class InventoryController {
  ...
  private getContext(req: RequestWithUser) {
    return {
  ...
  async createWarehouse(@Body() dto: CreateWarehouseDto, @Req() req: RequestWithUser) {
    return await this.inventoryService.createWarehouse(dto, this.getContext(req));
  }

  @Post('adjust')
  async adjustStock(@Body() dto: AdjustStockDto, @Req() req: RequestWithUser) {
    return await this.inventoryService.adjustStock(dto, this.getContext(req));
  }

  @Post('list')
  async listInventory(@Body() body: { warehouseId?: string }, @Req() req: RequestWithUser) {
    return await this.inventoryService.getInventoryLevels(body.warehouseId, this.getContext(req));
  }

  @Post('buffer/set')
  async setBuffer(@Body() dto: SetBufferDto, @Req() req: RequestWithUser) {
    return await this.inventoryService.setBuffer(dto, this.getContext(req));
  }

  @Post('layout/save')
  async saveLayout(@Body() dto: SaveLayoutDto, @Req() req: RequestWithUser) {
